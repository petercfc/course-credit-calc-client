...
const EditSubjectName = props => {
  ...
  const modal = useSelector(state => getModal(state, "editDegreeName"));

  //redux hook actions
  const dispatch = useDispatch();
  const toggleModal = useCallback(
    () => dispatch(modalOperations.toggleModal("editDegreeName")),
    []
  );
      ...
      {(updateDegree, { loading, error }) => {
        return (
          <Dialog open={modal.isOpen} onClose={handleDialogClose}>
            ...
            <DialogTitle>Edit Degree Name</DialogTitle>
            <DialogContent>
              <DialogContentText>
